...
El front no guarda datos ni aplica reglas de negocio: sus controladores llaman a un
servicio, el servicio pasa por `ApiClient` (que usa `HttpClient` del JDK) y la petición
llega por HTTP/JSON al recurso REST correspondiente en `ws`. Ahí la maneja un recurso
(`rest/`), que delega en un servicio (`service/`), que a su vez habla con un repositorio
(`repository/`, hoy implementado en memoria por `mocks/`).

Los dos lados comparten el módulo `shared/`: los mismos DTOs y enums se serializan de un
lado y se leen del otro, así que un cambio en un DTO se detecta al compilar en vez de
en tiempo de ejecución.

El front nunca decide nada importante: puede deshabilitar un botón para no ofrecer algo
que va a fallar, pero quien valida datos y permisos siempre es el web service.
...
1. Autenticación (`AuthenticationFilter`): valida que el token exista y no haya vencido.
   Sin token válido, 401.
2. Autorización (`AuthorizationFilter`): lee la anotación `@AllowedRoles` del método o de
   la clase y, si el rol del usuario no está en la lista, 403.
